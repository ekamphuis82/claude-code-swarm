...
| `topModel` | `null`, family alias, or pinned id (`null`) | `null` = top-tier calls inherit your session model. A family **alias** (`"opus"`/`"sonnet"`/`"haiku"`/`"fable"`) tracks the LATEST of that family — `"opus"` runs Opus 5 today and auto-upgrades when a newer Opus ships. A **pinned id** (`"claude-opus-4-8"`) freezes to that exact version. `"sonnet"` caps top-tier cost. Mechanical/verify tiers unaffected. No `"latest"` keyword — the alias is the latest-tracking value |
